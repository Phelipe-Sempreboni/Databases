-- Exemplo para verificar se uma tabela existe.
...
-- VII:
-- Para testar as duas condições do (PRINT), você pode utilziar o comando abaixo (DROP TABLE) que exclui a tabela criada no item (IV) para teste, logo, você testará com dados e sem dados.

BEGIN
	IF OBJECT_ID ('TESTE.testes.TabelaFuncionarios') IS NOT NULL
	PRINT('Tabela existe')
    ELSE
	PRINT('Tabela não existe')
END

-- Comando para teste e exluir tabela criada no item (IV).
BEGIN
DROP TABLE [TESTE].[testes].[TabelaFuncionarios]
END
