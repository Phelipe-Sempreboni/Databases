-- Comando para filtrar dados da tabela.
...
-- Utilizaremos neste caso a seleção de todas as colunas para realizar os filtros.

--------------------------------------------------------------------------------------------------------------------------------------

-- Filtrando um único ID.
SELECT * 
...
WHERE [ID] =271 OR [ID] = 20

--------------------------------------------------------------------------------------------------------------------------------------

-- FIltrando três ou mais ID com o comando OR.
SELECT * 

FROM [MARKETING].[MARKETING_ANALISE_CAMPANHA].[TBL_DADOS_CAMPANHA]

WHERE [ID] = 271 OR [ID] = 20 OR [ID] = 1


--------------------------------------------------------------------------------------------------------------------------------------

-- Filtrando três ou mais ID com o comando IN, logo, uma maneira mais fácil.

SELECT * 

FROM [MARKETING].[MARKETING_ANALISE_CAMPANHA].[TBL_DADOS_CAMPANHA]

WHERE [ID] IN (271, 1, 20)

--------------------------------------------------------------------------------------------------------------------------------------

-- Filtrando registros pelo ID que não contenham os valores (271, 1, 20), utilizamos o comando IN, porém, com a negação NOT, logo, NOT IN (Não contém).

SELECT * 

FROM [MARKETING].[MARKETING_ANALISE_CAMPANHA].[TBL_DADOS_CAMPANHA]

WHERE [ID] NOT IN (271, 1, 20)

--------------------------------------------------------------------------------------------------------------------------------------

-- Filtrando registros pelo ID maiores que 1.000.
...
-- Filtrando registros pelo ID menores que 1.000.
...
-- Filtrando registros entre 800 e 1.000.

SELECT * 

FROM [MARKETING].[MARKETING_ANALISE_CAMPANHA].[TBL_DADOS_CAMPANHA]

WHERE [ID] >= 800 AND [ID] <= 1000

--------------------------------------------------------------------------------------------------------------------------------------

-- Estes são alguns exemplos para filtrar registros de uma tabela.
